...
fn intended_cargo_targets(ws: &crate::tools::WorkspaceCap) -> bool {
...
fn run_verification(
    ...
    let (program, args) = if command == "cargo test --offline" {
        ...
        if !intended_cargo_targets(workspace) {
            return (
                false,
                "Cargo targets do not select local src/lib.rs and tests/roundtrip.rs".to_string(),
            );
        }
        (
            "cargo".to_string(),
