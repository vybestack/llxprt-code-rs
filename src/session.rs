...
pub struct SessionStore {
    ...
    context: Mutex<Option<ContextState>>,
}
...
const BULK_RESULT_BYTES: usize = 1024;
...
struct ContextState {
    store: crate::context_store::store::ContextStore,
    filters: crate::context_ingress::filter::FilterRegistry,
...
impl SessionStore {
    ...
    fn context_dir(&self) -> Result<openat::Dir, StoreError> {
        if let Ok(dir) = self.dir.sub_dir("context") {
            return Ok(dir);
        }
        ensure_private_subdir(&self.dir, "context")
    }

    /// Writes the sanitized spine, the vault snapshot, and the manifest under `context/`.
    fn persist_context(&self, state: &ContextState) -> Result<(), String> {
        let dir = self
            .context_dir()
            .map_err(|error| format!("open context directory failed: {error}"))?;
        let root = self.session_dir.join("context");
    ...
    fn record_quiesce_manifest(&self, state: &ContextState) {
        let manifest = ContextManifest {
            mode: state.store.mode().name(),
            quiesce: state.quiesce.as_deref(),
            detail: state.detail.as_deref(),
