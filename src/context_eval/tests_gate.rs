...
printf '{{"attempt":1,"branch":false,"branch_id":"fake-branch","budget_exhausted":false,"declared_tool_calls":-1,"prompt_digest":"{digest}","replayed":false,"session_dir":"/tmp/fake-sessions/%s","session_id":"%s","status":"ok","summary":"CTXEVAL final: CTXEVAL-FINAL-GATE","terminal_outcome":"wrap_up","tool_calls":0,"turn":%s}}\n' "$session" "$session" "$turn"
"#
    )
}
