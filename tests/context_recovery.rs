...
fn digest_record_names_the_floor_and_a_re_fetch_recipe() {
    ...
    let compacted = store
        ...
        .expect("compact tool result");
    assert_eq!(
        compacted.live, payload,
        "admitted live evidence is unchanged"
    );
    let compacted = compacted.persisted;
    assert!(
    ...
    let shell_compacted = store
        ...
        .expect("compact tool result");
    assert_eq!(shell_compacted.live, shell_payload);
    let shell_compacted = shell_compacted.persisted;
    assert!(
...
"
    ...
    assert_eq!(
        compacted.live, compacted.persisted,
        "drop verdict bounds both projections"
    );
    let compacted = compacted.persisted;
    assert!(
    ...
    let still = baseline_store
        .compact_tool_result("read_file", &payload)
        .expect("compact tool result");
    assert_eq!(still.live, payload, "baseline still admits live evidence");
    let still = still.persisted;
    assert!(
