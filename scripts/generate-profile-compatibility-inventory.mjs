...
function buildClassificationTable() {
  ...
  add('context-limit', host, common, 'host context limit; Codex requires a positive JSON integer');
...
function renderMarkdown(artifact) {
  ...
  push();
  push("Codex `context-limit` is a positive JSON integer, preserved as the agent's configured");
  push("context budget rather than constrained to a single model window. Zero and other types");
  push("reject. Enabled reasoning accepts `low`, `medium`, or `high` (the existing Responses");
  push("effort levels); the configured value is projected onto the Codex HTTP request with");
  push("summary `auto`. Disabled reasoning still requires effort and summary to be omitted.");
  push();
  ...
  push("seconds from 1 through 4294967295, or `-1`; other values reject explicitly. Omission");
  push("uses 120 seconds for each independently. `-1` disables that default timer or maximum");
  push("ceiling, not the other setting. A positive per-call `timeout_seconds` overrides the");
  push("default; the maximum then caps it. A disabled default with a finite maximum therefore");
  push("uses the maximum; both disabled means no shell timer unless a call supplies one.");
  push("A default above the maximum is capped, not rejected. Per-call zero rejects.");
  push("The unchanged astramedium default 2700 and maximum -1 mean a 2700-second default");
  push("with no ceiling on a positive per-call override. Request and turn deadlines remain");
  push("independent; shell settings neither replace nor disable them. Process-group cleanup,");
  push("output bounds and cancellation handling remain in force.");
  push("");
  push("`stream-first-response-timeout-ms` accepts only the");
  push("integer `-1` disabled sentinel (or omission): there is no separate first-response");
