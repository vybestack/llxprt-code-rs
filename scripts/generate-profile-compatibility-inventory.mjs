...
function buildClassificationTable() {
  ...
  add('shell-default-timeout-seconds', host, common, 'Codex: JSON number owned by the host, typed but inert in Rust; other APIs reject');
  add('shell-max-timeout-seconds', host, common, 'Codex: JSON number owned by the host, typed but inert in Rust; other APIs reject');
...
function renderMarkdown(artifact) {
  ...
  push();
  push("The current `astramedium` shape also includes host image-resize and shell settings.");
  push("`image-resize.maxLongEdge`, `image-resize.maxShortEdge`, `image-resize.maxPixels`,");
  push("`shell-default-timeout-seconds`, and `shell-max-timeout-seconds` accept the registry's");
  push("JSON number type as inert host data. This runtime has no image resizing pipeline or");
  push("host shell executor. Its own `run_shell_command` uses the runtime shell cap and the");
  push("bounded per-call `timeout_seconds`; these host settings do not override either.");
  push("Non-numeric values reject. `stream-first-response-timeout-ms` accepts only the");
